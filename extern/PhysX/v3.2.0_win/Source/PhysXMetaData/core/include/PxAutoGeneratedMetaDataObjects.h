...
template<> struct PxEnumTraits< physx::PxVisualizationParameter::Enum > { PxEnumTraits() : NameConversion( g_physx__PxVisualizationParameter__EnumConversion ) {} const PxU32ToName* NameConversion; }; 
...

	{
		...
		static PxU32 instancePropertyCount() { return 21; }
		static PxU32 totalPropertyCount() { return instancePropertyCount(); }
		template<typename TOperator>
		PxU32 visitInstanceProperties( TOperator inOperator, PxU32 inStartIndex = 0 ) const
		{
			...
			inOperator( TaskManager, inStartIndex + 3 );; 
			inOperator( SolverBatchSize, inStartIndex + 4 );; 
			inOperator( NbContactDataBlocks, inStartIndex + 5 );; 
			inOperator( MaxNbContactDataBlocksUsed, inStartIndex + 6 );; 
			inOperator( Aggregates, inStartIndex + 7 );; 
			inOperator( Actors, inStartIndex + 8 );; 
			inOperator( Articulations, inStartIndex + 9 );; 
			inOperator( Constraints, inStartIndex + 10 );; 
			inOperator( ClientBehaviorBits, inStartIndex + 11 );; 
			inOperator( ContactModifyCallback, inStartIndex + 12 );; 
			inOperator( FilterShaderData, inStartIndex + 13 );; 
			...
			inOperator( DynamicTreeRebuildRateHint, inStartIndex + 17 );; 
			inOperator( VisualizationCullingBox, inStartIndex + 18 );; 
			inOperator( UserData, inStartIndex + 19 );; 
			inOperator( SimulationStatistics, inStartIndex + 20 );; 
			return 21 + inStartIndex;
		}
	};
