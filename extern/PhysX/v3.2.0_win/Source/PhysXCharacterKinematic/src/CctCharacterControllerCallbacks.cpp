...
static void outputBoxToStream(PxShape* boxShape, const PxTransform& globalPose, IntArray& geomStream, const PxExtendedVec3& origin)
{
	...
	PxExtendedBox WorldBox;
	{
		...
		WorldBox.rot		= globalPose.q;
		WorldBox.extents	= bg.halfExtents;
		WorldBox.center.x	= globalPose.p.x;
		WorldBox.center.y	= globalPose.p.y;
		WorldBox.center.z	= globalPose.p.z;
	}

	TouchedBox* PX_RESTRICT touchedBox = (TouchedBox* PX_RESTRICT)reserve(geomStream, sizeof(TouchedBox)/sizeof(PxU32));
	...
	const CCTParams& params)
{
	...
	for(PxI32 i = 0; i < numberHits; i++)
	{
		...
		if(size_t(shape->userData)==PX_MAKE_FOURCC('C','C','T','S'))
			continue;
		...
		else	if(type==PxGeometryType::eBOX)			outputBoxToStream(shape, globalPose, geomStream, Origin);
