...
namespace Gu {
	...
	struct RTreePage {
		...
		enum { SIZE = 8 };
		PxU16 minx[8]; // [min=65535, max=0] is used as a sentinel range for empty bounds
		PxU16 miny[8];
		PxU16 minz[8];
		PxU16 maxx[8];
		PxU16 maxy[8];
		PxU16 maxz[8];
		PxU32 ptrs[8]; // for static rtree this is an offset relative to the first page divided by 16, for dynamics it's an absolute pointer divided by 16
	...
	struct PX_PHYSX_COMMON_API RTree
	{
		...
		bool load(PxInputStream& stream); // converts to proper endian at load time
	...
	protected:
		...
		bool		findObjectBackTrack(PxU32 objectHandle, const PxVec3& mn, const PxVec3& mx, NodeHandle* backtrackBuf, PxU32 bufsize) const;
				...
				PX_FORCE_INLINE RTreePage*	getPageFromNodeHandle(NodeHandle nh) const { return pagePtrFrom32Bits((nh*16)&~127); }
				PX_FORCE_INLINE PxU32		getNodeIdxFromNodeHandle(NodeHandle nh) const { return nh & 7; }
		static	PX_FORCE_INLINE PxU32		pagePtrTo32Bits(const RTreePage* page);
		static	PX_FORCE_INLINE RTreePage*	pagePtrFrom32Bits(PxU32 page);
				PX_FORCE_INLINE RTreePage*	get64BitBasePage() const;
		#ifdef PX_X64
	...
	#if BACKTRACK_HASH
	typedef Ps::HashMap<PxU32, PxU32> RTreeObjectMap;
	typedef Ps::HashMap<PxU32, PxU32> RTreePageMap;
	#endif

	struct PX_PHYSX_COMMON_API DynamicRTree : public RTree
	{
		struct NodeMarker { PxU16 x, y, z; }; // 6 bytes

		DynamicRTree(const PxVec3& worldMinBound, const PxVec3& worldMaxBound); // dynamic rtree constructor
	...
	private:
		...
		PX_FORCE_INLINE void relocateObjectHash(bool isBottomLevel, PxU32 objOrNodeHandle, PxU32 parentHandle, bool newItem);
	};
	...
	struct RTreeNodeQuantizer
	{
		...
		static PxVec3 computeInvDiagUpdateBounds(PxBounds3& treeBounds)
		{
			...
			// and have the [65535,0] inverted sentinel range used for empty nodes
