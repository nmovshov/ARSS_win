...
namespace Gu
{
	...
	PX_FORCE_INLINE PX_NOALIAS Ps::aos::Vec3V closestPtPointTriangle(const Ps::aos::Vec3VArg a, const Ps::aos::Vec3VArg b, const Ps::aos::Vec3VArg c, Ps::aos::Vec3V* PX_RESTRICT Q, Ps::aos::Vec3V* PX_RESTRICT A, Ps::aos::Vec3V* PX_RESTRICT B, PxU32& size, Ps::aos::Vec3V& closestA, Ps::aos::Vec3V& closestB)
	{
		using namespace Ps::aos;

		const FloatV zero = FZero();
		const FloatV one = FloatV_From_F32(0.999999994f);
		const BoolV bTrue = BTTTT();
		...
		const Vec3V nab = V3Normalize(ab);
		const Vec3V nac = V3Normalize(ac);
		const FloatV cosTheta = V3Dot(nab, nac);

		if(FAllGrtr(cosTheta, one))
		{
			size = 2;
			const Vec3V bc = V3Sub(c, b);
			const FloatV sqAb = V3Dot(ab, ab);
			const FloatV sqAc = V3Dot(ac, ac);
			const FloatV sqBc = V3Dot(bc, bc);
			const BoolV con0 = BAnd(FIsGrtrOrEq(sqBc, sqAc), FIsGrtrOrEq(sqBc, sqAb));
			const BoolV con1 = FIsGrtrOrEq(sqAc, sqAb);
	...
	PX_FORCE_INLINE PX_NOALIAS Ps::aos::Vec3V closestPtPointTriangleBaryCentric(const Ps::aos::Vec3VArg a, const Ps::aos::Vec3VArg b, const Ps::aos::Vec3VArg c, PxU32& size, Ps::aos::FloatV& t, Ps::aos::FloatV& w, PxU32* PX_RESTRICT indices)
	{
		using namespace Ps::aos;

		const FloatV zero = FZero();
		const FloatV oneEps = FloatV_From_F32(0.999999994f);
		const FloatV one = FOne();
		const BoolV bTrue = BTTTT();
		...
		if(FAllGrtr(cosTheta, oneEps))
		{
			...
			w = zero;
			return closestPtPointSegmentBaryCentric(Q0, Q1, size, t);
		}
