...
namespace Gu
{
	...
	*/
	...
	template<class ConvexA, class ConvexB>
	PxGJKStatus _GJKSeparatingAxis(const ConvexA& a, const ConvexB& b)
	{
		using namespace Ps::aos;
		const FloatV zero = FZero();
		const Vec3V zeroV = V3Zero();
	...
	PxGJKStatus _GJKSeparatingAxis(const ConvexV& a, const ConvexV& b, Support aSupport, Support bSupport);

	template<typename ConvexA, typename ConvexB>
	PxGJKStatus _GJKSeparatingAxis(const ConvexA& a, const ConvexB& b)
	{
		return _GJKSeparatingAxis(a, b, a.getSupportMapping(), b.getSupportMapping());
	}
