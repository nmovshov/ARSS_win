...
/*			return ((!(x < 0))
		...
		{
			//spu_printf("traceSegment\n");
			//spu_printf("aP0 = %.12f %.12f %.12f; aP1 = %.12f %.12f %.12f\n", aP0.x, aP0.y, aP0.z, aP1.x, aP1.y, aP1.z);
			...
			#endif
			{
				...
				//spu_printf("here1 ui=%d vi=%d\n", ui, vi);
				//spu_printf("h4=%.12f %.12f %.12f %.12f, minH=%.12f, maxH=%.12f, hPrev=%.12f, hNext=%.12f\n",
				...
				#endif
				{
					...
					if (!useUnderFaceCallback)
					{
						...
						//spu_printf("uhv0=%.12f %.12f %.12f, duhv=%.12f %.12f %.12f\n", uhv0.x, uhv0.y, uhv0.z, duhv.x, duhv.y, duhv.z);
						//spu_printf("p00a=%.12f %.12f %.12f, p01a=%.12f %.12f %.12f\n", p00a->x, p00a->y, p00a->z, p01a->x, p01a->y,  p01a->z);
						//spu_printf("p10a=%.12f %.12f %.12f, p11a=%.12f %.12f %.12f\n", p10a->x, p10a->y, p10a->z, p11a->x, p11a->y,  p11a->z);
						...
						if (hit0 && triT0 <= triT1)
						{
							...
							//spu_printf("callback triIndex0=%d\n", cellIndex*2);
						...
						else if (hit1 && triT1 <= triT0)
						{
							...
							//spu_printf("callback triIndex1=%d\n", cellIndex*2+1);
					...
					else
					{
						...
						//spu_printf("uf callback triIndex0=%d\n", triIndex0);
						...
						if (triIndex1 != triIndex0 && !isHole1) // if triIndex0 != triIndex1 that means we cross the triangle edge
						{
							...
							if (PxAbs(denom) > 1e-6f)
							{
								...
								//spu_printf("uf callback triIndex1=%d\n", triIndex1);
