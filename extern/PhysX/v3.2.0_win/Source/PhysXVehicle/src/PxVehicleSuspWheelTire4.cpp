...
void PxVehicleWheels4SimData::setSuspensionData(const PxVehicleSuspensionData& susp, const PxU32 id)
{
	...
	PX_CHECK_AND_RETURN(susp.mMaxCompression>0, "Susp max compression must be greater than zero");
	PX_CHECK_AND_RETURN(susp.mMaxDroop>0, "Susp max droop must be greater than zero");
...
void PxVehicleWheels4SimData::setWheelData(const PxVehicleWheelData& wheel, const PxU32 id)
{
	...
	PX_CHECK_AND_RETURN(wheel.mMaxSteer>=0, "Wheel max steer must be zero or be a positive value");
	PX_CHECK_AND_RETURN(wheel.mMaxSteer<PxHalfPi, "Wheel max steer must be in range (0,Pi/2)");
