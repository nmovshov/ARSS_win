%% A script to set up a freely spinning Rubble-Pile
% The nPhysX based sgpile program needs 2-3 input files. The .ini file is
% the general setup file used in all nPhysX simulations. The .ic file
% specifies the dynamical initial conditions - a list of positions to
% generate actors in and a matching list of starting velocities. The
% positions are used to create a rubble-pile with specific shape and
% compactness. The velocities are used to give the pile initial spin. An
% optional .sh file specifies individual particle shapes. If this file
% exists the number of shapes must match the number of actors derived from
% .ic, and the shapes are created in the corresponding position from .ic.
% If this file is missing generic shapes will be created at the .ic
% positions, based on the rubble_type and grain_size parameters in .ini.
% Unfortunately, there is some overlap in the information contained in
% these files. The number of actors is specified in .ini but implied in
% .ic. The grain size is specified in .ini but implied in .sh. To make sure
% these parameters match in a given simulation, it is best to create all 3
% files in a single script - this one.
%
...
restitution=0.8;
friction=0.5;
rhoBulk=9*si.g/si.cm^3; % bulk density (approx!!!)
% spin
saxis=[0 0 1]; % spin axis (remember orbit plane is xy)
T=2*si.hr; % spin period

%% ***Fresh Branch***
if freshSetup

%% more rubble parameters
nPart=0.5*1024; %#ok<UNRCH> % desired number of "particles"
...
% load from file,
if strcmp(tPart,'spheres') && exist('stdSpheres.mat','file') %#ok<UNRCH>
    load stdSpheres
